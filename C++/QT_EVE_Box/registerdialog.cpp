...
bool registerdialog::judgeEmpty()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("用户密码不能为空"));
        return false;
    }
    else if(ui->lineEdit_qq->text().isEmpty())
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("用户QQ号不能为空"));
        return false;
    }
    else if(ui->lineEdit_maill->text().isEmpty())
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("用户邮箱不能为空"));
        return false;
    }
    else if(ui->yanzhengline->text().isEmpty())
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("验证码不能为空"));
        return false;
    }
    else
        return true;
...
void registerdialog::on_registerbtn_clicked()
{
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("两次密码输入不一致"),QMessageBox::Yes);
        ui->passwordline->clear();
        ui->passwordline1->clear();
        ui->yanzhengline->clear();
        this->setyanzheng();
    ...
    {
        QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("验证码输入错误"),QMessageBox::Yes);
        ui->passwordline->clear();
        ui->passwordline1->clear();
        ui->yanzhengline->clear();
        this->setyanzheng();
    ...
    for(i=0;i<model->rowCount();i++)
    {
        ...
        {
            QMessageBox::warning(this,QStringLiteral("警告"),QStringLiteral("用户已存在"),QMessageBox::Yes);
            this->clearAll();
            this->setyanzheng();
            return;
            }
    ...
    if(model->submitAll())
    {
        QMessageBox::information(this,QStringLiteral("警告"),QStringLiteral("恭喜你注册成功^*^"),QMessageBox::Yes);
        this->accept();
    }
    //上传不成功
    else
    {
        QMessageBox::information(this,QStringLiteral("警告"),QStringLiteral("注册失败请重新注册"),QMessageBox::Yes);
    }
    this->clearAll();
    this->setyanzheng();
}
